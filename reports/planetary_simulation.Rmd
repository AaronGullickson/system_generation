...
#add number of recharge stations
generated_systems$recharge_nadir <- system$recharge$nadir
generated_systems$recharge_zenith <- system$recharge$zenith
...
for(i in 2:nrow(planets)) {
  ...
  rownames(temp) <- NULL
  temp$recharge_nadir <- system$recharge$nadir
  temp$recharge_zenith <- system$recharge$zenith
...
for(i in 1:nrow(generated_systems)) {
  ...
  psics <- project_sics(system$tech, system$industry, system$raw, 
                        system$output, system$agriculture,
                        system$founding_year, proj_pop, system$faction_type)
  projected_sics[[i]] <- data.frame(psics,
                                    faction_type=system$faction_type,
                                    x=system$x,
                                    y=system$y)
  recharge <- project_recharge(list(nadir=system$recharge_nadir,
                                    zenith=system$recharge_zenith),
                               system$faction_type,
                               system$founding_year, psics,
                               proj_pop)
  if(nrow(recharge)>0) {
    projected_recharge[[i]] <- data.frame(recharge,
                                          faction_type=system$faction_type,
                                          x=system$x,
                                          y=system$y)
  } else {
   projected_recharge[[i]] <- data.frame(year=NA,etype=NA,event=NA,
                                         faction_type=system$faction_type,
                                          x=system$x,
                                          y=system$y)
  }
...
We were getting around 10% for the Inner Shere whereas CamOps says it should be "under 25%" by 2820 so we added a +1 to the roll if planet was B or above in industry rating and a +1 if the planet was A-rated in tech. That gave us about 16%.

For projection, we added +3 to the roll for Start league peak, which gave us about 50% of worlds in the Inner Sphere with at least one recharge station and about 25% of major periphery planets with at least one recharge station.

```{r recharge, echo=FALSE}
extract_recharge <- function(x, chosen_year, type) {
  x <- na.omit(x)
  x <- subset(x, etype==type & year<=chosen_year)
  if(nrow(x)==0) {
    return(FALSE)
  }
  x <- x[order(x$year),]
  return(x[nrow(x),"event"])
}

recharge_data <- data.frame(stations=sapply(projected_recharge, extract_recharge,
                                            2750,"rechargeNadir")+
                              sapply(projected_recharge, extract_recharge, 
                                     2750,"rechargeZenith"),
                            year=2750,
                            faction_type=sapply(projected_recharge, function(x){x$faction_type[1]}),
                            x=sapply(projected_recharge, function(x){x$x[1]}),
                            y=sapply(projected_recharge, function(x){x$y[1]}))
recharge_data <- rbind(recharge_data,
                       data.frame(stations=sapply(projected_recharge, extract_recharge,
                                            2800,"rechargeNadir")+
                              sapply(projected_recharge, extract_recharge, 
                                     2800,"rechargeZenith"),
                            year=2800,
...
ggplot(recharge_data, aes(x=stations, y=..prop.., group=faction_type, fill=faction_type))+
...
```{r echo=FALSE}
#how do I get distribution of codes for a certain point in time?
extract_sics <- function(x, chosen_year) {
  temp <- x$year<=chosen_year
  if(sum(temp)==0) {
    return(rep(NA,8))
  }
  sics <- x$sics[max(which(temp))]
  sics <- separate_sics(as.character(sics))
  sics <- c(as.character(sics$tech), 
            as.character(sics$industry), 
            as.character(sics$raw), 
            as.character(sics$output), 
            as.character(sics$agriculture),
            as.character(x$faction_type[1]),
            x$x[1],
            x$y[1])
  return(sics)
}

get_sics_in_year <- function(chosen_year) {
  temp <- as.data.frame(na.omit(t(sapply(projected_sics, extract_sics, chosen_year))))
  colnames(temp) <- c("tech","industry","raw","output","agriculture","faction_type","x","y")
  temp$tech <- factor(temp$tech,
                      levels=c("X","F","D","C","B","A"),
                      ordered=TRUE)
  temp$industry <- factor(temp$industry,
                          levels=c("F","D","C","B","A"),
                          ordered=TRUE)
  temp$raw <- factor(temp$raw,
                     levels=c("F","D","C","B","A"),
                     ordered=TRUE)
  temp$output <- factor(temp$output,
                        levels=c("F","D","C","B","A"),
                        ordered=TRUE)
  temp$agriculture <- factor(temp$agriculture,
                             levels=c("F","D","C","B","A"),
                             ordered=TRUE)
...
full_sics <- NULL
for(y in seq(from=2200, to=3150, by=50)) {
  temp <- get_sics_in_year(y)
  temp$year <- y
  full_sics <- rbind(full_sics, temp)
}
```

```{r tech-project, echo=FALSE}
ggplot(full_sics, 
       aes(x=tech, y=..prop.., fill=year, group=year))+
  geom_bar(position = "dodge")+
  facet_wrap(~faction_type)+
  theme_bw()+
  labs(x="tech rating", y="proportion",
       title="Distribution of tech rating over time")
```

```{r tech-project-map, echo=FALSE}
ggplot(subset(full_sics, year %in% c(2200, 2400, 2600, 2650, 2700, 2750, 2900, 3000, 3050, 3100)), 
       aes(x=x, y=y, color=tech))+
  geom_point(alpha=0.2)+
  theme_minimal()+
  facet_wrap(~year)+
  scale_color_brewer(palette="Set1")+
  labs(title="Tech ratings over time", x=NULL, y=NULL)+
  theme(axis.ticks=element_blank(),
        axis.text = element_blank())
```

```{r industry-project, echo=FALSE}
ggplot(full_sics, 
       aes(x=industry, y=..prop.., fill=year, group=year))+
  geom_bar(position = "dodge")+
  facet_wrap(~faction_type)+
  theme_bw()+
  labs(x="industry rating", y="proportion",
       title="Distribution of industry rating over time")
```

```{r industry-project-map, echo=FALSE}
ggplot(subset(full_sics, year %in% c(2200, 2400, 2600, 2650, 2700, 2750, 2900, 3000, 3050, 3100)), 
       aes(x=x, y=y, color=industry))+
  geom_point(alpha=0.2)+
  theme_minimal()+
  facet_wrap(~year)+
  scale_color_brewer(palette="Set1")+
  labs(title="Industry ratings over time", x=NULL, y=NULL)+
  theme(axis.ticks=element_blank(),
        axis.text = element_blank())
```

```{r raw-project, echo=FALSE}
ggplot(full_sics, 
       aes(x=raw, y=..prop.., fill=year, group=year))+
  geom_bar(position = "dodge")+
  facet_wrap(~faction_type)+
  theme_bw()+
  labs(x="raw material rating", y="proportion",
       title="Distribution of raw material rating over time")
```

```{r raw-project-map, echo=FALSE}
ggplot(subset(full_sics, year %in% c(2200, 2400, 2600, 2650, 2700, 2750, 2900, 3000, 3050, 3100)), 
       aes(x=x, y=y, color=raw))+
  geom_point(alpha=0.2)+
  theme_minimal()+
  facet_wrap(~year)+
  scale_color_brewer(palette="Set1")+
  labs(title="Raw material ratings over time", x=NULL, y=NULL)+
  theme(axis.ticks=element_blank(),
        axis.text = element_blank())
```

```{r output-project, echo=FALSE}
ggplot(full_sics, 
       aes(x=output, y=..prop.., fill=year, group=year))+
  geom_bar(position = "dodge")+
  facet_wrap(~faction_type)+
  theme_bw()+
  labs(x="output rating", y="proportion",
       title="Distribution of output rating over time")
```

```{r output-project-map, echo=FALSE}
ggplot(subset(full_sics, year %in% c(2200, 2400, 2600, 2650, 2700, 2750, 2900, 3000, 3050, 3100)), 
       aes(x=x, y=y, color=output))+
  geom_point(alpha=0.2)+
  theme_minimal()+
  facet_wrap(~year)+
  scale_color_brewer(palette="Set1")+
  labs(title="Output ratings over time", x=NULL, y=NULL)+
  theme(axis.ticks=element_blank(),
        axis.text = element_blank())
```

```{r ag-project, echo=FALSE}
ggplot(full_sics, 
       aes(x=agriculture, y=..prop.., fill=year, group=year))+
  geom_bar(position = "dodge")+
  facet_wrap(~faction_type)+
  theme_bw()+
  labs(x="agriculture rating", y="proportion",
       title="Distribution of agriculture rating over time")
```

```{r ag-project-map, echo=FALSE}
ggplot(subset(full_sics, year %in% c(2200, 2400, 2600, 2650, 2700, 2750, 2900, 3000, 3050, 3100)), 
       aes(x=x, y=y, color=tech))+
  geom_point(alpha=0.2)+
  theme_minimal()+
  facet_wrap(~year)+
  scale_color_brewer(palette="Set1")+
  labs(title="Agriculture ratings over time", x=NULL, y=NULL)+
  theme(axis.ticks=element_blank(),
        axis.text = element_blank())
```
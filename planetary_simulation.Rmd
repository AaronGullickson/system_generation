...
projected_populations <- matrix(NA, nrow(generated_systems), length(2100:3060))
colnames(projected_populations) <- paste(2100:3060)
rownames(projected_populations) <- paste(generated_systems$name)
for(i in 1:nrow(generated_systems)) {
  system <- generated_systems[i,]
  proj_pop <- project_population(system$population, 3060, system$founding_year, system$faction_type)
  projected_populations[i,paste(system$founding_year:3060)] <- proj_pop
}
```
...
plot(2100:3060, total_pop, type="l", lwd=3, xlab="year", ylab="total human population")
```
...
ggplot(generated_systems, aes(x=x, y=y,
  ...
  geom_point(alpha=0.8)+
  theme_minimal()+
    scale_color_brewer(palette="BuGn", name="Depopulation Quartiles", direction=-1)+
...
hist(starting_size[starting_size<1000000],
...
hist(max_pop, 
     ...
     xlab="Maximum population size on planet")
```

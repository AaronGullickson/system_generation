...
generated_systems$recharge <- system$recharge$nadir+system$recharge$zenith
...
for(i in 2:nrow(planets)) {
  ...
  rownames(temp) <- NULL
  temp$recharge <- system$recharge$nadir+system$recharge$zenith
...
for(i in 1:nrow(generated_systems)) {
  ...
  projected_sics[[i]] <- data.frame(project_sics(system$tech, system$industry, system$raw, 
...
tapply(generated_systems$recharge>0, generated_systems$faction_type, mean)
ggplot(generated_systems, aes(x=recharge>0, y=..prop.., 
                              group=faction_type, fill=faction_type))+
